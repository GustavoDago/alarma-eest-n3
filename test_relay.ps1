# Script para probar el Rele USB (LCUS-1 / CH340) en Windows
# Este script activa el rele dos veces para verificar su funcionamiento fisico.
...
function Test-Relay {
    ...
    # Si no se especifica puerto, intentamos encontrar el CH340 automaticamente
    if (-not $PortName) {
        ...
        if ($dev) {
            # Extraer el COM port del nombre amigable (ej: "USB-SERIAL CH340 (COM3)")
            # Usamos comillas simples para la regex para evitar problemas de escape
            if ($dev.FriendlyName -match '\((COM\d+)\)') {
                $PortName = $Matches[1]
                Write-Host "OK: Encontrado en puerto: $PortName" -ForegroundColor Green
            }
        }
    }

    if (-not $PortName) {
        Write-Host "ERROR: No se encontro el rele automaticamente." -ForegroundColor Red
        Write-Host "Por favor, verifica el Administrador de Dispositivos y ejecuta el script indicando el puerto:"
        Write-Host "Ejemplo: .\test_relay.ps1 -PortName COM3"
        return
    }
    ...
    try {
        Write-Host "Conectando al puerto $PortName..."
        $serial.Open()
        ...
        Write-Host "`nPrueba completada con exito." -ForegroundColor Green
    }
    catch {
